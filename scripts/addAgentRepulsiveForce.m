...
for fi = 1:data.floor_count
    ...
    for ai = 1:agents_on_floor
        ...
        idx = rangeQuery(tree, pi(1) - r_max, pi(1) + r_max, pi(2) - r_max, pi(2) + r_max)';
        %idx = rangeQuery(tree, pi(1) - r_max*3, pi(1) + r_max*3, pi(2) - r_max*3, pi(2) + r_max*3)';
        ...
        for aj = idx
            ...
            if aj > ai
                ...
                % repulsive forces canceled to reduce impact on colission 
                T1 = 0;
                T2 = 0;
                ...
                cosij = dot(data.floor(fi).agents(ai).f,data.floor(fi).agents(aj).f) ...
                          /(norm(data.floor(fi).agents(ai).f)*norm(data.floor(fi).agents(aj).f));

                % Compute avoidance angle, 22.5 deg in proportion to the cosine similarity
                theta = 5 * abs(cosij);

                % Compute rotation matrix
                Ri = [cosd(theta) -sind(theta); sind(theta) cosd(theta)];
                Rj = [cosd(-theta) -sind(-theta); sind(-theta) cosd(-theta)];

                % Rotate force vector to avoid collision 
                data.floor(fi).agents(ai).f = data.floor(fi).agents(ai).f * Ri;    
                data.floor(fi).agents(aj).f = data.floor(fi).agents(aj).f * Rj;    
